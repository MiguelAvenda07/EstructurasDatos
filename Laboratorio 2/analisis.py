...
def medir_tiempo_cercano():
    ...
    for n in tamanios:
        ...
        for objetivo in objetivos:
            inicio = time.time()
            encontrar_mas_cercano(arbol, objetivo)
    ...
    return tamanios, tiempos_kdtree, tiempos_bruta
...
def medir_tiempo_radio():
    ...
    for radio in radios:
        ...
        for centro in centros:
            inicio = time.time()
            encontrar_radio(arbol, centro, radio)
            t_kd.append(time.time() - inicio)
...
def graficar_rendimiento():
    tamanios, t_kd_cercano, t_fb_cercano = medir_tiempo_cercano()
    radios,   t_kd_radio,   t_fb_radio   = medir_tiempo_radio()

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))

    # Gráfico 1: punto más cercano
    ax1.plot(tamanios, t_kd_cercano, marker='o', color='blue',   label='KDtree')
    ...
    ax2.plot(radios, t_kd_radio, marker='o', color='blue',   label='KDtree')
    ...
    plt.suptitle('KDtree vs Fuerza bruta', fontsize=13)
    plt.tight_layout()
    plt.show()

    # Imprimir resumen
    print("=== Punto más cercano ===")
    print(f"{'Tamaño':<10} {'KDtree (s)':<15} {'Fuerza bruta (s)':<18}")
    for i in range(len(tamanios)):
        print(f"{tamanios[i]:<10} {t_kd_cercano[i]:<15.6f} {t_fb_cercano[i]:<18.6f}")
